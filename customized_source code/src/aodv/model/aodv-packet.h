...
public:
  ...
  static TypeId GetTypeId ();
  TypeId GetInstanceTypeId () const;
  uint32_t GetSerializedSize6 () const;
  void Serialize6 (Buffer::Iterator start) const;
  uint32_t Deserialize6 (Buffer::Iterator start);
  void Print6 (std::ostream &os) const;

  // Fields
  void SetHopCount6 (uint8_t count) { m_hopCount6 = count; }
  uint8_t GetHopCount6 () const { return m_hopCount6; }
  void SetId6 (uint32_t id) { m_requestID6 = id; }
  uint32_t GetId6 () const { return m_requestID6; }
  void SetDst6 (Ipv6Address a) { m_dst6 = a; }
  Ipv6Address GetDst6 () const { return m_dst6; }
  void SetDstSeqno6 (uint32_t s) { m_dstSeqNo6 = s; }
  uint32_t GetDstSeqno6 () const { return m_dstSeqNo6; }
  void SetOrigin6 (Ipv6Address a) { m_origin6 = a; }
  Ipv6Address GetOrigin6 () const { return m_origin6; }
  void SetOriginSeqno6 (uint32_t s) { m_originSeqNo6 = s; }
  uint32_t GetOriginSeqno6 () const { return m_originSeqNo6; }

  // Flags
  void SetGratiousRrep6 (bool f);
  bool GetGratiousRrep6 () const;
  void SetDestinationOnly6 (bool f);
  bool GetDestinationOnly6 () const;
  void SetUnknownSeqno6 (bool f);
  bool GetUnknownSeqno6 () const;

  bool operator== (RreqHeader const & o) const;
private:
  uint8_t        m_flags6;          ///< |J|R|G|D|U| bit flags, see RFC
  uint8_t        m_reserved6;       ///< Not used
  uint8_t        m_hopCount6;       ///< Hop Count
  uint32_t       m_requestID6;      ///< RREQ ID
  Ipv4Address    m_dst6;            ///< Destination IP Address
  uint32_t       m_dstSeqNo6;       ///< Destination Sequence Number
  Ipv4Address    m_origin6;         ///< Originator IP Address
  uint32_t       m_originSeqNo6;    ///< Source Sequence Number
};
